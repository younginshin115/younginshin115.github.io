...
[1] dags 폴더에 예제 파일을 작성합니다. 파일 제목은 print_data.py로 합니다.
...
[3] DB를 초기화 하고 나면 dag list 명령어로 dags 폴더에 넣어두었던 DAG가 DAG 리스트에 등록된 것을 확인할 수 있습니다.
{% highlight shell linenos %}
airflow dags list
{% endhighlight %}
...
[4] 아래 명령어로 특정 DAG 안에 포함된 Task들도 확인할 수 있습니다.
{% highlight shell linenos %}
...
[5] 테스트는 airflow tasks test {DAG ID} {TASK_ID} {기준 날짜} 명령어를 사용하여 Task 별로 시행할 수 있습니다.
{% highlight shell linenos %}
...
[6] DAG 등록과 테스트가 완료되었다면 airflow scheduler를 실행시킵니다. 그러면 등록된 DAG가 자동으로 실행됩니다.
{% highlight shell linenos %}
airflow scheduler
{% endhighlight %}
...
[7] 웹 UI에 접속하면 아래와 같이 등록된 DAG와 그 상태를 확인할 수 있습니다.
