...
- 주요 성능 최적화 사례:
  ...
  - 배치 인퍼런스 및 데이터 적재 최적화로 수백만 건 데이터 분석 시간 7일→1일 단축
...
### 모델 정량 평가 파이프라인 최적화 | 2023.03 – 2023.08
- 평가 서버 GPU 부재로 인한 외부 의존성 및 성능 문제 해결
- 키-값 기반 임베딩 캐싱 시스템 구현으로 학습-평가 단계 분리 → 학습 시 ID별 임베딩을 파일 저장, 평가 시 저장된 임베딩 활용
- CPU 기반 예측으로 전환하여 GPU 인프라 비용 절감 및 평가 시간 65% 단축
- 외부 서버 호출 제거로 평가 파이프라인 독립성 확보 → 운영 안정성 향상

**기술 스택**: Python, NumPy, Flask, TensorFlow/PyTorch
...
- MediaPipe Pose 기반 실시간 운동 자세 분석 코칭 서비스 구현
- Bazel을 활용하여 MediaPipe 기반 모델을 AAR 형태로 패키징, 네트워크 독립적인 온디바이스 AI 환경 구축
- 실시간 영상 처리 및 포즈 인식 알고리즘 최적화로 모바일 환경 내 저지연 피드백 시스템 구현

**기술 스택**: Android Studio, Java, MediaPipe, Bazel, OpenCV, TensorFlow Lite

### 음성 기반 점수 평가 자동화 시스템 구축 | 2021.10 – 2021.11
- 기존 수작업 음성-점수 변환 프로세스를 전 과정 자동화하여 인적 의존성 제거
- Pipeline: 영상 내 오디오 추출 → STT(Speech-to-Text) 변환 → Classification 모델 학습 및 예측으로 이어지는 데이터 워크플로우 설계.
...
**GitHub**: 
- 모델 개발: [lucid-ddos](https://github.com/younginshin115/lucid-ddos)
- 전체 시스템: [prism](https://github.com/younginshin115/prism)
...
**Blog**: [FitGuru](/project/project-04-fitguru/)
...
**GitHub**: [Anomaly_Prediction](https://github.com/younginshin115/Anomaly_Prediction)  
...
**GitHub**: [21_hyu_javas](https://github.com/younginshin115/21_hyu_javas)   
...
**GitHub**: [21_hyu_algomeokjo](https://github.com/younginshin115/21_hyu_algomeokjo) 
